                           ...
                           sdb_read_null_mode read_null_mode)
{
   ...
   bool start_key_part_used_and_key_isnull = (start_key_part_map & 1)
                                             && key_part->null_bit
                                             && *start_key_ptr;
   bool end_key_part_used_and_key_isnull = (end_key_part_map & 1)
                                             &&  key_part->null_bit
                                             && *end_key_ptr;
        ...
        ++key_part )
   {
      ...
      if(key_part_temp == key_part)
      {
         rc = build_start_end_key_obj(start_key_ptr,
                                    ...
                                    start_key_obj,
                                    end_key_obj,
                                    (key_part == key_end - 1 || start_key_part_map == 1) ? last_part_op_str : pre_k_1_parts_op_str,
                                    end_key_op_str,
                                    end_find_flag,
                                    read_null_mode);
         if(rc)
                                                     ...
                                                     int *order_direction)
{
   ...
   *order_direction = get_key_direction(start_match_mode);
   /*multiple-column index which have k parts, pre_k_1_parts_op_str is the first k-1 parts op_str.
    For the query
        SELECT MIN(key_part_k)
        FROM t1
        WHERE key_part_1 >= const and ... and key_part_k >= const
    when build start obj, if the op_str is "$gte", then pre_k_1_parts_op_str should be "$gt",
    $or: $gt:{k_part_1:const1}
         $et:{k_part_1:const1}, $gt:{k_part_2:const2}
         ...
         $et:{k_part_1:const1}, $et:{k_part_2:const2},...,$et:{k_part_k-1:constk-1},$gte:{k_part_k:constk}
    as same, if the op_str is "$lte", then pre_k_1_parts_op_str should be "$lt",
   */
